...
const CONFIG = {
  ...
  embeddingDimensions: 512, // Match your Pinecone index dimension
...
async function uploadToPinecone(embeddings) {
  ...
  for (let i = 0; i < vectors.length; i += CONFIG.batchSize) {
    ...
    console.log(`  Uploading batch ${Math.floor(i / CONFIG.batchSize) + 1}/${Math.ceil(vectors.length / CONFIG.batchSize)} (${batch.length} vectors)...`);
    ...
    try {
      ...
      // Pinecone SDK v7 expects { records: [...] } format
      ...
      console.error('Batch sample:', JSON.stringify(batch[0], null, 2).substring(0, 500));
      throw error;
    }
  }
